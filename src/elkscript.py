...
def elk_create_new_index(index_name, config):
    ...
    with open("mapping.json", 'r', encoding="UTF-8") as file:
        ...
        response = requests.put(elk_url+"/" + index_name, headers=headers,
...
def elk_post_logs_bulk(json_file_path, index, config):
    ...
    with open(json_file_path, 'r', encoding="UTF-8") as file:
        ...
        for log in logs:
            ...
            data_string = data_string + json.dumps(log) + "\n"
        response = requests.post(elk_url+"/"+index+"/_bulk", headers=headers,
                                 data=data_string)
        return response
