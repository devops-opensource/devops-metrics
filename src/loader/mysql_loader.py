...
class MySqlLoader:
    def __init__(self, config):
        ...
        db_data = ("{engine}://{user}:{pw}@{host}/{db}"
        .format(host=self._host, engine=self._engine, user=self._username, pw=self._password, db=self._database))
    ...
    def loadReleasesFromDf(self, df):
        dtype = {
            "release_date": sqlalchemy.DateTime, "start_date": sqlalchemy.DateTime,
            "control_date": sqlalchemy.DateTime
        }
        df.to_sql(name='releases', con=self._engine, if_exists='replace', index=False, dtype=dtype)

    def loadStatusChangesFromDf(self, df):
        dtype = {
            "to_date": sqlalchemy.DateTime, "from_date": sqlalchemy.DateTime,
            "control_date": sqlalchemy.DateTime
        }
