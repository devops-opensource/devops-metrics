...
class TransformStatusChanges:
    ...
    def transform_status_changes(self, df_status_changes):
        ...
        df_status_changes["from_date"] = df_status_changes.sort_values(
            ["to_date"]).groupby("key")["to_date"].shift()
        df_status_changes["from_date"] = df_status_changes["from_date"].fillna(
            df_status_changes["creation_date"])
        df_status_changes.loc[df_status_changes.sort_values(["to_date"]).groupby(
            "key")["from_status"].head(1).index, "from_status"] = self._creation_status
    ...
    def add_transition_to_released_status(self, df_status_changes):
        df_closed = df_status_changes[df_status_changes["to_status"].isin(
            self._closed_statuses)]
        df_closed_dedup = df_closed.loc[df_closed.groupby(
            "key")["to_date"].transform("max") == df_closed["to_date"]]
        ...
        for row in df_closed_dedup.itertuples():
            ...
            dates = df_version[df_version["name"].isin(
                versions)][["release_date", "name"]].dropna()
            if len(dates) != 0:
